...
def fetch_data(url, tag="", filter_recent_months=False, is_json=False):
    try:
        ...
        if is_json:
            ...
            if content[:4] == b'PK\x03\x04':
                ...
                with zipfile.ZipFile(io.BytesIO(content)) as z:
                    ...
                    if csv_infos:
                        # 優先鎖定容量最大的單一 CSV 主檔（避免合併到結構不同的輔助 CSV）
                        target_info = max(csv_infos, key=lambda x: x.file_size)
                        print(f"[{tag}] 鎖定單一主資料檔: {target_info.filename} (大小: {target_info.file_size / 1024 / 1024:.2f} MB)")
                        df = pd.read_csv(z.open(target_info.filename), on_bad_lines='skip', encoding='utf-8-sig', dtype=str)
        ...
        # --- 時間過濾 (僅對有設定 filter_recent_months 的事故實施) ---
...
if pts_a1:
    fg1 = folium.FeatureGroup(name="🚨 A1 類重大交通事故 (全)", show=True)
...
m.save("index.html")
print("更新完成！A1 改為全抓，A2 鎖定容量最大單一主檔！")
